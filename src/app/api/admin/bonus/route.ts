...
const createNewBonus = await Bonus.create(formDetails)

if (createNewBonus) {
    return NextResponse.json({data:createNewBonus,success:true,message:"you have successfully added new bonus"})
}
else{
    return NextResponse.json({data:null,success:false,message:"cant add bonus now,please try again later"})
}
...
export const GET = async()=>{
    ...
    try {
    const getBonuses = await Bonus.find({})
    if (!getBonuses) {
        return NextResponse.json({message:"an error ocurred , can't fetch bonuses",success:false})
        
    }

    return NextResponse.json({data:getBonuses,success:true})
