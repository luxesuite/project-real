...
const postData = async(formDetails:any)=>{
    const res = await fetch(`/api/admin/withdrawal/delete-withdrawal`,{
    method:"POST",
    headers:{
...
// purchase is used for the withdrawal state coming from the parent compoent
  const [purchases, setPurchases] = useState<any[]>(allWithdrawals);
  ...
  return (
    <div className="max-w-full p-6">
      {/* Header and Actions */}
            ...
            onClick={()=>{
                router.push("/admin/accounts/withdrawal/add")
            }}
            >Add Withdrawal</button>
        </div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold">Withdrawal Records</h2>
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow">
        <div className="min-w-[700px] w-full">
          ...
          <div className="flex bg-gray-100 p-4 border-b border-gray-300 text-sm font-medium">
            ...
            <div className="w-[120px] px-4 flex items-center">Confirmed</div>
          </div>
...
setHistoryItems([...historyItems,item])
              ...
              <div className="w-[120px] px-4 flex items-center justify-center">
                {item.confirmed === "yes" ? <IoIosCheckmarkCircle className='text-green-500 text-lg' /> : <MdCancel className='text-red-500 text-lg' /> }
              </div>
            </div>
          ))}
          ...
          {currentPurchases.length === 0 && (
            <div className="flex p-6 text-center text-gray-500">
              ...
              <div className="flex-1">No Withdrawal found</div>
            </div>
          )}
        </div>
      </div>
