...
const postData = async(formDetails:any)=>{
    const res = await fetch(`/api/admin/deposit/delete-deposit`,{
    method:"POST",
    headers:{
...
const DepositComponent = ({allDeposits}:{allDeposits:any}) => {
    ...
    onSuccess:(data)=>{
        ...
        if (data.success) {
           historyItems.forEach((item:any) => {
                      ...
                      postHistory({username:item.username,actionPerformed:"delete",action:"deposit"})
                    });
...
// purchase is used for the deposit state coming from the parent compoent
  const [purchases, setPurchases] = useState<any[]>(dateSort(allDeposits));
  ...
  return (
    <div className="max-w-full p-6">
      {/* Header and Actions */}
            ...
            onClick={()=>{
                router.push("/admin/accounts/deposit/add")
            }}
            >Add Deposit</button>
        </div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold">Deposit Records</h2>
        ...
        <div className="min-w-[700px] w-full">
          ...
          {currentPurchases.length === 0 && (
            <div className="flex p-6 text-center text-gray-500">
              ...
              <div className="flex-1">No Deposit found</div>
            </div>
          )}
        </div>
      </div>
